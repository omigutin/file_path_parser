...
def test_parse_simple_groups():
    parser = FilePathParser(["cat", "dog"], ["night", "day"])
    result = parser.parse("cat_night.jpg")
    assert result["group1"] == "cat"
    assert result["group2"] == "night"
...
def test_parse_path_priority_filename_vs_path():
    ...
    # filename has "aaa", dirpath has "bbb"
...
def test_parse_with_dict_group_and_case_insensitivity():
    ...
    result = parser.parse("something_blue.txt")
    assert result["color"] == "Blue"
    result2 = parser.parse("something_red.txt")
